...
LocalPlayer.OnTeleport:Connect(function(state)
    queue_on_teleport([[
        ...
        HumanoidRootPart = Character:WaitForChild("HumanoidRootPart")
        Humanoid = Character:WaitForChild("Humanoid")
        loadstring(game:HttpGet('https://raw.githubusercontent.com/hyonkey/txt-ohioffts/refs/heads/main/ohioeivid.lua'))()
    ]])
end)
...
workspace.Game.Entities.ItemPickup.DescendantAdded:Connect(function(des)
    if des:IsA("ProximityPrompt") then
        ...
        if des.ObjectText and des.ObjectText ~= "" then
            ...
            task.spawn(function()
                local waitTime = 0
                while waitTime < 0.3 and des and des.Parent do
                    if des.ObjectText and des.ObjectText ~= "" then
                        manageItemPickupsTable(des)
                        break
                    end
