...
local scriptId = "non_printer_collector"
    ...
    local success, err = pcall(function()
        ...
        trackConnection(LocalPlayer.OnTeleport:Connect(function(state)
            queue_on_teleport([[
                ...
                loadstring(game:HttpGet('https://raw.githubusercontent.com/hyonkey/txt-ohioffts/refs/heads/main/ohiogoldenrose.lua'))()
            ]])
        end))
        ...
        local absoluteTimeout = 120
        local startTime = os.time()
        local postPickupTimer = 5
        local serverTimerStart = os.time()
        local currentState = "INITIAL_WAIT"
        local pickupWaitStart = 0
        local hasItemsDetected = false
        ...
        pickupTask = task.spawn(function()
            while task.wait() do
                ...
                if HumanoidRootPart and Humanoid and Humanoid.Health > 0 then
                    ...
                    for i, v in pairs(ItemPickups) do
                        if isValidPickup(v) then
                            ...
                            if objectText and PermanentSet[objectText] then
                                if objectText ~= "Candy Cane" and objectText ~= "Money Printer" then
                    ...
                    if foundValidPermanent then
                        hasItemsDetected = true
                        
                        if currentState == "INITIAL_WAIT" then
                            currentState = "PICKUP_ACTIVE"
                        end
                    else
                        hasItemsDetected = false
                    end

                    if currentState == "PICKUP_ACTIVE" then
                        ...
                        currentState = "POST_PICKUP_WAIT"
                        pickupWaitStart = os.time()
                    end
                end
            end
        end)
        ...
        while task.wait(0.1) do
            ...
            local serverTimeElapsed = currentTime - serverTimerStart

            local playerCount = #Players:GetPlayers()
            local initialTimer
            if playerCount < 10 then
                initialTimer = 50
            elseif playerCount <= 20 then
                initialTimer = 45
            else
                initialTimer = 50
            end
            ...
            if playerCount < 3 then
                warn("Server player count less than 3, forcing server switch")
                itemdone = true
            end

            if currentTime - startTime >= absoluteTimeout then
                warn("Reached absolute timeout, forcing server switch")
                itemdone = true
            end

            if currentState == "INITIAL_WAIT" then
                if serverTimeElapsed >= initialTimer then
                    if not hasItemsDetected then
                        warn("No permanent items detected within " .. initialTimer .. " seconds, switching server")
                        itemdone = true
                    end
                end
            elseif currentState == "POST_PICKUP_WAIT" then
                if currentTime - pickupWaitStart >= postPickupTimer then
                    if not hasItemsDetected then
                        warn("No new items detected within " .. postPickupTimer .. " seconds after pickup, switching server")
                        itemdone = true
                    else
                        currentState = "PICKUP_ACTIVE"
                    end
